        ...
        try:
            for i in jobseekerDataFrame.index:
                ...
                for j in jobsDataFrame.index:
                    ...
                    jobseekerSkills = [
                        x.lower()
                        for x in list(jobseekerDataFrame["skills"][i].split(", "))
                    ]
                    jobsSkills = [
                        x.lower()
                        for x in list(jobsDataFrame["required_skills"][j].split(", "))
                    ]
                    ...
                    newData = [
                        jobseekerDataFrame["id"][i],
                        jobseekerDataFrame["name"][i],
                        jobsDataFrame["id"][j],
                        jobsDataFrame["title"][j],
                        matchingSkillCount,
                        matchingSkillPercentage,
                    ]
                    combineNewData.append(newData)

                ## Sort the list by Matching Skill Percentage
                combinedData.extend(
                    sorted(combineNewData, key=itemgetter(5), reverse=True)
                )

                ## Sort the list by Job ID if Matching Skill Percentage are same
                for i in range(len(combinedData) - 1):
                    if (combinedData[i][2] > combinedData[i + 1][2]) and (
                        combinedData[i][5] == combinedData[i + 1][5]
                    ):
